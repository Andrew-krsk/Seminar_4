...
public class Team<T extends Warrior> {
    ...
    public int getMinGuard() {
        int guard = 500;
        for (T item : team) {
            if (!(item instanceof Guard)) {
                continue;
            }
            Guard temp = (Guard) item;
            if (temp.getGuard() < guard) {
                guard = temp.getGuard();
            }
        }
        return guard;
    ...
    public String toString() {
        ...
        return String.format("Team{ team= %s, maxDistance = %d, maxDamage = %d, minGuard = %d \n%s}",
                name, getMaxDistance(), getTeamDamage(), getMinGuard(), teamBuilder);
    }
}
